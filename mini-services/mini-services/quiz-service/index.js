...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', (data) => {
    const settings = data.settings || { category: 'Mixte', questionCount: 10, timerDuration: 10 };
  ...
  socket.on('start-game', () => {
    for (const [, room] of rooms) {
      if (room.hostId === socket.id && room.status === 'lobby') {
        ...
        room.players.forEach(p => { p.score = 0; p.currentAnswer = null; p.answerTime = null; });
      ...
      if (room.hostId === socket.id && room.status === 'playing') {
        if (room.questionState === 'reading') {
          ...
          if (currentQ) {
            room.players.forEach(p => {
              if (p.currentAnswer === currentQ.correct && p.answerTime && room.timerStartsAt) {
                ...
                p.score += 500 + Math.floor(speedBonus * 500);
              }
            });
          }
          ...
          if (room.currentQuestionIndex >= room.questions.length - 1) {
            ...
            room.questionState = 'reading';
            room.timerStartsAt = null;
            room.players.forEach(p => { p.currentAnswer = null; p.answerTime = null; });
  ...
  socket.on('kick-player', (data) => {
    for (const [, room] of rooms) {
      if (room.hostId === socket.id) {
        ...
        if (p && data.playerId !== socket.id) {
          room.players.delete(data.playerId);
          io.to(data.playerId).emit('kicked', { message: 'Vous avez été expulsé.' });
          broadcastRoomState(room);
          console.log(`[KICKED] ${p.name} from room ${room.code}`);
        }
        break;
      }
    }
      ...
      if (player) {
        ...
        if (socket.id === room.hostId) {
          room.players.forEach((p, pid) => {
            if (pid !== socket.id) {
              io.to(pid).emit('error', { message: "L'hôte s'est déconnecté." });
            }
          });
          rooms.delete(code);
